...
@onready var sound_space: ColorRect = $SoundSpace  # Make sure this path matches your scene
...
func _process(delta):
	...
	if not rect.has_point(current_touch_pos):
		return  # Ignore touches outside the sound_space

	# Normalize position inside sound_space
	...
	var pitch = lerp(200.0, 1000.0, normalized_x)
	var vol = lerp(0.1, 1.0, 1.0 - normalized_y)
